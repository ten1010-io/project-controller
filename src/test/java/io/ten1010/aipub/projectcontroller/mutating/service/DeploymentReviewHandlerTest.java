...
class DeploymentReviewHandlerTest {

  private static final String EXCLUSION_LABEL = "test.aipub/excluded";

  private DeploymentReviewHandler handler;
  ...
  void setUp() {
    ...
    this.handler = new DeploymentReviewHandler(this.mockNodesResolver, factory,
        reconciliationService);
    this.mapper = new ObjectMapperFactory().createObjectMapper();
  }

  private V1AdmissionReview createReview(String namespace, Map<String, String> deploymentLabels) {
    V1Deployment deployment = new V1Deployment()
        .metadata(new V1ObjectMeta()
            .name("test-deployment").namespace(namespace).labels(deploymentLabels))
    ...
    request.setObject(this.mapper.valueToTree(deployment));
  ...
  @Test
  @DisplayName("allowlist 네임스페이스의 deployment에는 템플릿 toleration으로 Exists 쌍 패치를 주입한다")
  void handle_allowlistedNamespace_injectsExistsTolerationPatch() {
    ...
    assertThat(patch).contains("/spec/template/spec/tolerations");
  ...
  @Test
  @DisplayName("제외 라벨이 붙은 deployment라도 allowlist 네임스페이스면 toleration 패치를 주입한다(allowlist 우선)")
  void handle_excludedDeploymentInAllowlistedNamespace_stillInjectsToleration() {
  ...
  @Test
  @DisplayName("allowlist가 아닌 네임스페이스에서 제외 라벨 deployment는 무패치 허용한다")
  void handle_excludedDeploymentInOrdinaryNamespace_allowsWithoutPatch() {
