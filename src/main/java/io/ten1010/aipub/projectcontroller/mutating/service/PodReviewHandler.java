...
public class PodReviewHandler extends AbstractReviewHandler<V1Pod> {
    ...
    public void handle(V1AdmissionReview review) {
        ...
        V1alpha1Project project = this.projectIndexer.getByKey(new KeyResolver().resolveKey(K8sObjectUtils.getNamespace(pod)));

        List<V1Toleration> reconciledTolerations = this.reconciliationService.reconcileTolerations(pod, allowedProjectNodeObjects);
        List<V1NodeSelectorTerm> reconciledSelectorTerms = this.reconciliationService.reconcileNodeSelectorTerms(pod, project);
        List<V1LocalObjectReference> reconciledImagePullSecrets = this.reconciliationService.reconcileImagePullSecrets(pod, project);
