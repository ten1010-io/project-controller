...
public class DeploymentReviewHandler extends AbstractReviewHandler<V1Deployment> {
    ...
    public DeploymentReviewHandler(
            WorkloadControllerNodesResolver workloadControllerNodesResolver, SharedInformerFactory sharedInformerFactory, ReconciliationService reconciliationService) {
    ...
    public void handle(V1AdmissionReview review) {
        ...
        List<V1Toleration> reconciledTolerations = this.reconciliationService.reconcileTolerations(podTemplateSpec, nodeObjects);
        List<V1NodeSelectorTerm> reconciledSelectorTerms = this.reconciliationService.reconcileNodeSelectorTerms(podTemplateSpec, project);
        List<V1LocalObjectReference> reconciledImagePullSecrets = this.reconciliationService.reconcileImagePullSecrets(podTemplateSpec, project);
        ...
        if (!reconciledSelectorTerms.isEmpty()) {
            V1Affinity reconciledAffinity = new V1AffinityBuilder(deployment.getSpec().getTemplate().getSpec().getAffinity())
                    .editNodeAffinity()
