...
public class AdmissionReviewController {
  ...
  private static WebExceptionHandler createWebExceptionHandler() {
    ...
    new AdmissionReviewExceptionWebMetadataRegistrar(backendMetadataRegistry).register(
        frontendMetadataRegistry);

    LoggerProvider loggerProvider = new Slf4jLoggerProvider(
        AdmissionReviewController.class.getSimpleName() + "."
            + WebExceptionHandler.class.getSimpleName());
    return new WebExceptionHandler(frontendMetadataRegistry, loggerProvider);
  }

  private static SpringWebExceptionHandlerAdapter createExceptionHandlerAdapter(
    ...
    SpringWebExceptionHandlerAdapter handlerAdapter = new SpringWebExceptionHandlerAdapter(
        exceptionHandler);
  ...
  private V1AdmissionReview getV1AdmissionReview(WebRequest request)
    ...
    if (!(servletWebRequest.getNativeRequest() instanceof ContentCachingRequestWrapper nativeRequest)) {
      throw new IllegalArgumentException(
          "nativeRequest must be an instance of ContentCachingRequestWrapper");
    }
