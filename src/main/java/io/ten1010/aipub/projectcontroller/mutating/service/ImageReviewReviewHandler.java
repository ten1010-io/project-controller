...
import io.ten1010.aipub.projectcontroller.domain.k8s.dto.V1alpha1ImageReviewStatus;
import io.ten1010.aipub.projectcontroller.domain.k8s.dto.V1alpha1ReviewArtifact;
import io.ten1010.aipub.projectcontroller.domain.k8s.dto.V1alpha1ReviewRepository;
...
public class ImageReviewReviewHandler extends AbstractReviewHandler<V1alpha1ImageReview> {
  ...
  private final static V1OwnerReference DUMMY_REF = new V1OwnerReferenceBuilder()
      .withApiVersion(K8sObjectTypeConstants.IMAGE_REVIEW_V1ALPHA1.typeKey().getApiVersion())
      .withKind(K8sObjectTypeConstants.IMAGE_REVIEW_V1ALPHA1.typeKey().getKind())
  ...
  private boolean denyIfUnauthorized(V1AdmissionReview review, String targetImgHub) {
    ...
    if (boundImageHubs.stream().noneMatch(e -> e.equals(targetImgHub))) {
      V1AdmissionReviewUtils.reject(review, HttpStatus.FORBIDDEN.value(),
          String.format("Aipub member not bound to ImageHub with name[%s]", targetImgHub));
      return true;
    }
  ...
  private void doService(V1AdmissionReview review, String targetImgHub,
      @Nullable String targetRepo) {
    V1alpha1ImageHub imageHub = this.imageHubIndexer.getByKey(
        this.keyResolver.resolveKey(targetImgHub));
    if (imageHub == null) {
      V1AdmissionReviewUtils.reject(review, HttpStatus.BAD_REQUEST.value(),
