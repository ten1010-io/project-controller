...
public class ClusterRoleBindingControllerFactory implements ControllerFactory {
  ...
  public Controller createController() {
    return ControllerBuilder.defaultBuilder(this.sharedInformerFactory)
        ...
        .withReadyFunc(this.sharedInformerFactory.getExistingSharedIndexInformer(
            V1ClusterRoleBinding.class)::hasSynced)
        .withReadyFunc(this.sharedInformerFactory.getExistingSharedIndexInformer(
            V1alpha1Project.class)::hasSynced)
        .watch(this::createClusterRoleBindingWatch)
        .watch(this::createProjectWatch)
        .withReconciler(
            new ClusterRoleBindingReconciler(this.sharedInformerFactory, this.k8sApiProvider,
                this.reconciliationService))
        .build();
  }

  private ControllerWatch<V1ClusterRoleBinding> createClusterRoleBindingWatch(
      WorkQueue<Request> workQueue) {
    DefaultControllerWatch<V1ClusterRoleBinding> watch = new DefaultControllerWatch<>(workQueue,
  ...
  private ControllerWatch<V1alpha1Project> createProjectWatch(WorkQueue<Request> workQueue) {
    ...
    watch.setRequestBuilder(this.requestBuilderFactory.projectToProjectRoles(false));
    return watch;
  }
