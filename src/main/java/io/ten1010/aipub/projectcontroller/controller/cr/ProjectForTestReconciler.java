...
public class ProjectForTestReconciler extends AbstractReconciler {

    private final ReconciliationService reconciliationService;
    private final Indexer<V1alpha1ProjectForTest> projectIndexer;
    ...
    private final GenericKubernetesApi<V1alpha1ProjectForTest, V1alpha1ProjectForTestList> projectApi;
    private final StatusPatchHelper<V1alpha1ProjectForTest> statusPatchHelper;
    private final KeyResolver keyResolver;
    private final NamespaceNameResolver namespaceNameResolver;

    public ProjectForTestReconciler(
            ReconciliationService reconciliationService,
            SharedInformerFactory sharedInformerFactory,
            K8sApiProvider k8sApiProvider) {
        this.reconciliationService = reconciliationService;
        this.projectIndexer = sharedInformerFactory
                .getExistingSharedIndexInformer(V1alpha1ProjectForTest.class)
                .getIndexer();
        ...
        this.projectApi = k8sApiProvider.getProjectForTestApi();
        this.statusPatchHelper = new StatusPatchHelper<>(
                k8sApiProvider.getApiClient(),
                K8sObjectTypeConstants.PROJECT_FOR_TEST_V1ALPHA1,
    ...
    protected Result reconcileInternal(Request request) throws ApiException {
        ...
        Optional<V1alpha1ProjectForTest> projectOpt = Optional.ofNullable(this.projectIndexer.getByKey(projectKey));
        if (projectOpt.isEmpty()) {
            return new Result(false);
        }
        V1alpha1ProjectForTest project = projectOpt.get();
        ...
        List<V1alpha1ImageHub> boundImageHubs = this.boundObjectResolver.getAllBoundImageHubsForTest(project);
        V1alpha1ProjectStatus reconciledStatus = this.reconciliationService.reconcileProjectForTestStatus(
    ...
    private Result reconcileTerminatingProject(V1alpha1ProjectForTest project, boolean namespaceRemoved) throws ApiException {
        if (namespaceRemoved) {
            V1alpha1ProjectForTest clone = ProjectUtils.clone(project);
    ...
    private Result reconcileExistingProject(V1alpha1ProjectForTest project, V1alpha1ProjectStatus reconciledStatus) throws ApiException {
    ...
    @Nullable
    private V1ResourceQuota getBoundResourceQuota(V1alpha1ProjectForTest project) {
        String projName = K8sObjectUtils.getName(project);
        String quotaName = new ResourceQuotaNameResolver().resolveQuotaName(projName);
        String quotaKey = this.keyResolver.resolveKey(projName, quotaName);
        return this.quotaIndexer.getByKey(quotaKey);
