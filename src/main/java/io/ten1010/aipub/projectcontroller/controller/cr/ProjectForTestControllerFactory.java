...
import io.ten1010.aipub.projectcontroller.domain.k8s.dto.V1alpha1ProjectForTest;
import lombok.AllArgsConstructor;

@AllArgsConstructor
public class ProjectForTestControllerFactory implements ControllerFactory {
    ...
    public ProjectForTestControllerFactory(
    ...
    public Controller createController() {
        return ControllerBuilder.defaultBuilder(this.sharedInformerFactory)
                .withName("project-for-test-controller")
                .withWorkerCount(1)
                .withReadyFunc(this.sharedInformerFactory.getExistingSharedIndexInformer(V1alpha1ProjectForTest.class)::hasSynced)
                .withReadyFunc(this.sharedInformerFactory.getExistingSharedIndexInformer(V1Namespace.class)::hasSynced)
                ...
                .withReconciler(new ProjectForTestReconciler(this.reconciliationService, this.sharedInformerFactory, this.k8sApiProvider))
                .build();
    }

    private ControllerWatch<V1alpha1ProjectForTest> createProjectWatch(WorkQueue<Request> workQueue) {
        DefaultControllerWatch<V1alpha1ProjectForTest> watch = new DefaultControllerWatch<>(workQueue, V1alpha1ProjectForTest.class);
        watch.setOnUpdateFilter(this.onUpdateFilterFactory.projectForTestSpecFieldFilter());
        return watch;
    }

    private ControllerWatch<V1Namespace> createNamespaceWatch(WorkQueue<Request> workQueue) {
        DefaultControllerWatch<V1Namespace> watch = new DefaultControllerWatch<>(workQueue, V1Namespace.class);
