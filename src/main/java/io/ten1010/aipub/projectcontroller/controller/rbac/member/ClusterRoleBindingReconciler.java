...
public class ClusterRoleBindingReconciler extends AbstractReconciler {
  ...
  protected Result reconcileInternal(Request request) throws ApiException {
    ...
    Optional<V1ClusterRoleBinding> roleBindingOpt = Optional.ofNullable(
        this.clusterRoleBindingIndexer.getByKey(roleBindingKey));
    ...
    List<V1OwnerReference> reconciledReferences = this.reconciliationService.reconcileOwnerReferences(
        roleBindingOpt.orElse(null), projectOpt.get());
    V1RoleRef reconciledRoleRef = this.reconciliationService.reconcileClusterRoleRef(
        projectOpt.get(), projRoleEnum);
    List<RbacV1Subject> subjects = this.reconciliationService.reconcileSubjects(projectOpt.get(),
        projRoleEnum);
    ...
    if (!K8sObjectUtils.isTerminating(projectOpt.get())) {
      return reconcileNoExistingRoleBinding(request.getName(), reconciledReferences,
          reconciledRoleRef, subjects);
    }
  ...
  private Result reconcileExistingRoleBinding(
    ...
    if (Set.copyOf(K8sObjectUtils.getOwnerReferences(clusterRoleBinding))
        .equals(Set.copyOf(reconciledReferences)) &&
        clusterRoleBinding.getRoleRef().equals(reconciledRoleRef) &&
        Set.copyOf(RoleUtils.getSubjects(clusterRoleBinding))
  ...
  private void updateRoleBinding(String objName, V1ClusterRoleBinding roleBinding)
      throws ApiException {
    this.rbacAuthorizationV1Api
