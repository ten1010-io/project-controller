...
public class AipubUserReconciler extends AbstractReconciler {
  ...
  protected Result reconcileInternal(Request request) throws ApiException {
    ...
    Optional<V1alpha1AipubUser> userOpt = Optional.ofNullable(
        this.aipubUserIndexer.getByKey(aipubUserKey));
    if (userOpt.isEmpty()) {
    ...
    V1alpha1AipubUserStatus reconciledStatus = this.reconciliationService.reconcileAipubUserStatus(
        user, boundProjects, boundHubs);

    return reconcileExistingAipubUser(userOpt.get(), reconciledStatus);
  }

  private Result reconcileExistingAipubUser(V1alpha1AipubUser existing,
      V1alpha1AipubUserStatus reconciledStatus) throws ApiException {
  ...
  private void updateStatus(V1alpha1AipubUser aipubUser) throws ApiException {
    ...
    this.statusPatchHelper.patchStatus(null, K8sObjectUtils.getName(aipubUser),
        aipubUser.getStatus());
  }
