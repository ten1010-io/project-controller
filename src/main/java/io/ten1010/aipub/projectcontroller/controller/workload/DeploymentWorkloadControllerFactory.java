...
public class DeploymentWorkloadControllerFactory extends WorkloadControllerFactory {
  ...
  protected void configureReadyFunc() {
    this.builder.withReadyFunc(
        this.sharedInformerFactory.getExistingSharedIndexInformer(V1Deployment.class)::hasSynced);
  ...
  private Result reconcileController(
    ...
    updateDeployment(K8sObjectUtils.getNamespace(controller), K8sObjectUtils.getName(controller),
        edited);
    return new Result(false);
  }

  private void updateDeployment(String namespace, String objName, V1Deployment deployment)
      throws ApiException {
    this.appsV1Api
  ...
  private ControllerWatch<V1Deployment> createDeploymentWatch(WorkQueue<Request> workQueue) {
    DefaultControllerWatch<V1Deployment> watch = new DefaultControllerWatch<>(workQueue,
        V1Deployment.class);
  ...
  private ControllerWatch<V1alpha1Project> createProjectWatch(WorkQueue<Request> workQueue) {
    ...
    watch.setRequestBuilder(
        this.requestBuilderFactory.projectToNamespacedObjects(V1Deployment.class));
    return watch;
  }
  ...
  private ControllerWatch<V1alpha1NodeGroup> createNodeGroupWatch(WorkQueue<Request> workQueue) {
    ...
    watch.setRequestBuilder(
        this.requestBuilderFactory.nodeGroupToNamespacedObjects(V1Deployment.class));
    return watch;
  }
