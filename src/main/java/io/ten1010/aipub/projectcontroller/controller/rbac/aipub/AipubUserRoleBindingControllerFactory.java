...
public class AipubUserRoleBindingControllerFactory implements ControllerFactory {
  ...
  public AipubUserRoleBindingControllerFactory(
  ...
  public Controller createController() {
    return ControllerBuilder.defaultBuilder(this.sharedInformerFactory)
        .withName("aipub-user-role-binding-controller")
        .withWorkerCount(1)
        .withReadyFunc(this.sharedInformerFactory.getExistingSharedIndexInformer(
            V1RoleBinding.class)::hasSynced)
        .withReadyFunc(this.sharedInformerFactory.getExistingSharedIndexInformer(
        ...
        .withReconciler(
            new AipubUserRoleBindingReconciler(this.sharedInformerFactory, this.k8sApiProvider,
                this.reconciliationService))
        .build();
  }

  private ControllerWatch<V1RoleBinding> createRoleBindingWatch(WorkQueue<Request> workQueue) {
    DefaultControllerWatch<V1RoleBinding> watch = new DefaultControllerWatch<>(workQueue,
        V1RoleBinding.class);
    watch.setOnUpdateFilter(this.onUpdateFilterFactory.aipubUserRoleBindingFilter());
    return watch;
  }
