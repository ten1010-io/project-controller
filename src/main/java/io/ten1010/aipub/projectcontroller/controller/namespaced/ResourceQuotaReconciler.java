...
public class ResourceQuotaReconciler extends AbstractReconciler {
  ...
  protected Result reconcileInternal(Request request) throws ApiException {
    ...
    List<V1OwnerReference> reconciledReferences = this.reconciliationService.reconcileOwnerReferences(
        quotaOpt.orElse(null), projectOpt.get());
    V1ResourceQuotaSpec reconciledQuotaSpec = this.reconciliationService.reconcileQuotaSpec(
        projectOpt.get());
    ...
    if (!K8sObjectUtils.isTerminating(projectOpt.get())) {
      return reconcileNoExistingQuota(request.getNamespace(), request.getName(),
          reconciledReferences, reconciledQuotaSpec);
    }
  ...
  private Result reconcileExistingQuota(
      ...
      List<V1OwnerReference> reconciledReferences,
      V1ResourceQuotaSpec reconciledQuotaSpec) throws ApiException {
    if (Set.copyOf(K8sObjectUtils.getOwnerReferences(quota))
  ...
  private void updateQuota(String namespace, String objName, V1ResourceQuota quota)
      throws ApiException {
    this.coreV1Api
